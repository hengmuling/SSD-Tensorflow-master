...
def inception_v3_base(inputs, scope=None):
    ...
    with tf.variable_scope(scope, 'InceptionV3', [inputs]):
        ...
        with slim.arg_scope([slim.conv2d, slim.max_pool2d, slim.avg_pool2d],
            ...
            with tf.variable_scope('Mixed_7a'):  # third inception layer
                 with tf.variable_scope('Branch_0'):
                      branch_0 = slim.conv2d(net, 192, [1, 1], scope='Conv2d_0a_1x1')
...
def inception_v3(inputs,
                 ...
                 reuse=None,
                 scope='Inceptionv3'):
    with tf.variable_scope(scope, 'InceptionV3', [inputs, num_class],
...
def time_tensorflow(session, target, info_string):
    ...
    # after circle is finished
    mm = total_duration / num_batch
    vr = total_duration_squared / num_batch - mm * mm
    sd = math.sqrt(vr)
    print('%s:%s across %d steps, %.3f +/- %.3f sec/batch' % (datetime.now(), info_string, num_batch, mm, sd))
...
sess = tf.Session()
sess.run(init)
time_tensorflow(sess, logits, "Forward")
